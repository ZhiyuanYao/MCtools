...
struct TimeStat TimeStatistics(double *datas[], unsigned int datasize)
{
 ...
 * ====================================================================================*/
struct BlockStat BlockStatistics(double *datas[], unsigned int datasize)
{
    ...
    */
    ...
    for (k = 0; k < N_VAR; k++) {
        ...
        if ( bs[k] == NULL) {
            stop("bs[k] allocation failed!\n");
        }
    }

    for (k = 0; k < N_VAR; k++) {
        pts[k] = & datas[k][datasize - n];
    ...
    for (int k = 0; k < N_VAR; k++)  free(bs[k]);
    return (stat);
}
 ...
 * ====================================================================================*/
struct BootStrapStat BootStrapStatistics(double *datas[], unsigned int datasize)
{
    ...
    */
    ...
    for (times = 0; times < BOOTMAX; m *= 2, times++) {
        sum1 = 0.0; sum2 = 0.0;              /* sum1, sum2 is for sums of single bootstrap value */
        for (i = 0; i < m; i++)              /* m : # of bootstrap draw */
        {
            for (k = 0; k < N_VAR; k++)
                sums[k] = 0.0;

            for (j = 0; j < n; j++)           /* draw data n times from the n samples  */
            {
                n1 = (int) (rndm() * n);
                for (k = 0; k < N_VAR; k++)
                    sums[k] += bs[k][n1];
